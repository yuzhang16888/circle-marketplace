...
# Try to import stripe, but don't crash the whole app if it's missing
try:
...
def render(user):
    ...
    order_id = create_order(
        ...
        payment_method=payment_method,  # now we store selected method
    ...
    # ---------- IF STRIPE LIB IS NOT AVAILABLE (Streamlit Cloud demo) ----------
    if stripe is None:
        st.success("✅ Order created.")
        st.info(
            "Online card payment isn't enabled in this demo deployment. "
            "Please follow the seller's instructions to pay via "
            "Zelle, PayPal, or the method you selected above."
        )
        return

    # ---------- IF STRIPE *IS* AVAILABLE: CREATE CHECKOUT SESSION ----------
    # For now this still uses your existing Stripe flow (for local dev or once Stripe is installed in cloud)
    try:
    ...
    # NOTE: This part still expects a seller_stripe_account_id.
    # Once we fully switch to Model 2, we'll refactor this to pay into Circle's platform account instead.
