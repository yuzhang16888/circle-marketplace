...
# Engine for SQLite (Streamlit uses multiple threads, so we need this flag)
...
def get_db():
    ...
    In Streamlit, you'll typically do:

        db = get_db()
        db.query(User)...

    For now we won't worry about explicit close() in dev.
    """
    return SessionLocal()
...
def init_db():
    ...
    app.py is already importing and calling this.
    """
    # Import models so that they are registered with Base.metadata
    from core import models  # this imports core/models/__init__.py
